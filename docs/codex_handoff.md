...
最新状态：ARKRenderer 当前代码实现已完成 Phase 0.47。material ball visual fixture 已接入 asset、GltfLoader、ModelResource、RenderQueue 和真实 Vulkan frame validation smoke；下一阶段应优先推进 tone-mapped LDR readback、screenshot/golden validation 或 public scene/resource loading API。
...
- 当前仍不是完整 production IBL；已具备最小 HDR scene color readback / statistical frame validation 和 material ball visual fixture，但尚缺完整 screenshot/golden image 验证、tone-mapped LDR readback、public quality config API、shadow、多光源、bloom 或 auto exposure。
...
- 当前仍缺完整 screenshot/golden image system、tone-mapped LDR readback、public quality config API、shadow、多光源、bloom 和 auto exposure。
...
- cubemap RHI/Vulkan 语义、equirectangular -> cubemap conversion、diffuse irradiance generation、ForwardPass diffuse IBL、face orientation debug foundation、最小 GPU readback / pixel validation、face-mip render target view foundation、prefiltered specular generator foundation、BRDF LUT foundation、默认 renderer specular bake path、ForwardPass specular IBL、roughness/metallic validation fixture、默认 specular quality policy 和最小 HDR scene color statistical frame validation 已存在，但当前没有 cubemap CPU upload、cubemap mip generation policy、tone-mapped LDR readback 或 screenshot/golden validation。
...
Phase 0.47 已完成 material ball visual fixture foundation。下一阶段建议沿着“最终可见画面验证”继续推进，并开始整理 renderer/sandbox 的 public scene loading 边界：
...
- 若继续推进视觉质量：在有 LDR readback/golden 前，不建议先扩 Bloom 或 auto exposure；先保证当前 PBR/IBL 输出能稳定被最终画面验证捕捉。

不要直接跳到 RenderGraph、bindless、完整材质扩展或复杂后处理栈。

优先顺序：

1. Tone-mapped LDR readback，基于 `ToneMappingPass` 输出做 CPU 统计，补齐 HDR scene color 与用户最终画面的验证差异。
2. Screenshot / golden image system，基于稳定相机、quad-grid/material-ball fixtures 和 readback harness 扩展 PNG/baseline/image diff。
3. Public scene/resource loading API，把默认模型、环境、相机和材质资源加载从 sandbox 特例整理成公开入口。
...
补充：当前最新已完成 Phase 0.47，新增 `assets/models/material_ball_validation_fixture.gltf`，覆盖 15 个材质球、稳定 `MaterialBallCamera`、共享 sphere accessors、loader/model/resource/render queue coverage，`ark_frame_validation_smoke` 已同时渲染 specular quad-grid fixture 和 material ball fixture；后续不应再把材质球视觉 fixture 作为缺口。
