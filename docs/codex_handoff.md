...
- Hotfix: a later sandbox startup crash showed Vulkan validation `VUID-VkShaderModuleCreateInfo-pCode-08740` for `DemoteToHelperInvocation`. `mesh.frag.hlsl` alpha-mask `discard` compiles to that SPIR-V capability, so `VulkanDevice` now queries Vulkan 1.3 device features and enables `shaderDemoteToHelperInvocation` alongside dynamic rendering, failing early if unsupported. Revalidated targeted build/CTest, full Debug build, full CTest 31/31, and sandbox default / `--no-ui` / `--preset shadow-validation` launch smoke.
- Hotfix: default UI startup was still crashing after environment bake while `--no-ui` stayed stable. Root cause was mixing `VK_NO_PROTOTYPES + volk` global C symbols with the vcpkg `imgui::imgui` Vulkan backend built against `vulkan-1`; `vkGetDeviceProcAddr` resolved to volk's global function-pointer data symbol and ImGui's dynamic-rendering loader tried to call that data address. The project now compiles its own `src/rhi/vulkan/Volk.cpp` with `VOLK_NAMESPACE` and links only `volk::volk_headers`, keeping volk symbols namespaced while ImGui continues using the Vulkan loader path. Revalidated default UI and `--no-ui` hidden-window smoke, targeted UI/dependency/header CTests, full Debug build, and full CTest 31/31.
- Hotfix: sandbox runtime tuning of Shadow Map Extent and Bloom parameters triggered `VK_NOT_READY` / `VK_ERROR_DEVICE_LOST`. Root cause was eager destruction of in-flight runtime targets when `ShadowPass`, `BloomPass`, or `FrameRenderer` resized their textures/views/samplers. Fixed by routing runtime target replacement through `DeviceContext::deferRelease*`, limiting Bloom target rebuilds to extent / mip-structure changes, and keeping intensity/threshold/scatter/soft-knee as uniform-only updates. Revalidated full Debug build, targeted `ark_bloom_pass_smoke` and `ark_shadow_pass_smoke`, full CTest 31/31, and sandbox default UI hidden-window smoke stayed alive for 8s.
...
最新状态：ARKRenderer 当前代码实现已完成 Phase 0.48。`ark_frame_validation_smoke` 已在真实 Vulkan command flow 中新增 tone-mapped `RGBA8Unorm` LDR offscreen target/readback，保留原有 `RGBA16Float` HDR scene color readback，并让 specular IBL quad-grid fixture 与 material ball fixture 同时通过 HDR/LDR 统计校验；下一阶段应优先推进 screenshot/golden validation 或 public scene/resource loading API。
...
Phase 0.47 补充：新增 `assets/models/material_ball_validation_fixture.gltf`，覆盖 15 个材质球、稳定 `MaterialBallCamera`、共享 sphere accessors、loader/model/resource/render queue coverage，并让 `ark_frame_validation_smoke` 同时渲染旧 quad-grid fixture 和新 material ball fixture。Windows/MSVC/vcpkg/DXC debug preset 下 targeted smoke、full build、CTest 22/22、default sandbox smoke 和 material ball fixture sandbox smoke 均已通过。

Phase 0.48 补充：`ark_frame_validation_smoke` 已新增 `RGBA8Unorm` LDR offscreen render target、LDR readback buffer 和 LDR frame stats，真实 Vulkan 验证会先读取 HDR scene color，再把 scene color 转为 shader resource 并执行 `ToneMappingPass`，最后读取 tone-mapped LDR 输出。SkyboxPass、ForwardPass、ToneMappingPass 的测试作用域已延长到 GPU idle 后，避免 command buffer 仍引用提前析构的 pass 私有资源。Windows/MSVC/vcpkg/DXC debug preset 下 targeted frame/tone-mapping smoke、full build、CTest 22/22 和 default sandbox hidden-window smoke 均已通过。
...
Specular IBL validation fixture 入口：
...
- `assets/models/specular_ibl_validation_fixture.gltf` 提供 15 个 quad-grid material / mesh / node，覆盖 metallic `{0.0, 0.5, 1.0}` 和 roughness `{0.05, 0.25, 0.5, 0.75, 1.0}`，用于 specular IBL validation。
- `gltf_loader_smoke` / `model_resource_smoke` 已覆盖该 fixture 的 material factors、baseColor texture、render state、ModelResource counts 和 RenderQueue 15 draw 展开。
- `forward_pass_pipeline_smoke` 已覆盖 15 draw material grid 的 per-material roughness / metallic uniform、specular IBL enable flag 和 max mip。
- 当前默认 specular IBL quality policy 已文档化：prefiltered specular cube `256x256 RGBA16Float` full mip chain（9 mips）、prefilter sample count `128`、BRDF LUT `256x256 RGBA16Float`、BRDF LUT sample count `1024`。
- 当前仍不是完整 production IBL；已具备最小 HDR scene color readback / statistical frame validation、tone-mapped LDR readback 和 material ball visual fixture，但尚缺完整 screenshot/golden image 验证、public quality config API、shadow、多光源、bloom 或 auto exposure。
...
```text
f16efca Add ForwardPass specular IBL
...
本轮 0.45.0 ~ 0.45.6 收尾主要工作区改动：

```text
## main...origin/main
 M CMakeLists.txt
 M docs/codex_handoff.md
 M src/app/SandboxCameraController.cpp
 M src/rhi/vulkan/VulkanCommandContext.cpp
 M tests/readback_api_smoke.cpp
 M tests/sandbox_camera_controller_smoke.cpp
...
- 当前仍缺 screenshot/golden image validation、frame color readback validation、glTF camera、shadow、多光源、bloom 和 auto exposure；roughness/metallic quad-grid fixture 和默认 quality policy 已在 Phase 0.44 补齐。

### Phase 0.44（0.44.0 ~ 0.44.6 已完成并验证）

- 新增 `docs/phase/phase44.md`，明确本阶段只做 specular IBL validation / quality pass，不做完整 screenshot/golden image、RenderGraph、后处理栈或新材质扩展。
- 新增 `assets/models/specular_ibl_validation_fixture.gltf`，使用 15 个 quad-grid material / mesh / node 覆盖 metallic `{0.0, 0.5, 1.0}` 和 roughness `{0.05, 0.25, 0.5, 0.75, 1.0}`。
- fixture 复用既有 `assets/textures/xiaowei.png` 作为 baseColor texture，符合当前 `GltfLoader` material baseline；roughness / metallic 仍由 material factors 驱动，不使用 metallicRoughness texture。
- `ark_gltf_loader_smoke` 已覆盖 fixture shape、material names、baseColor texture、metallic/roughness factors、opaque render state 和 node transform。
- `ark_model_resource_smoke` 已覆盖 fixture 进入 `ModelResource` 后的 primitive/material/instance counts、fallback texture reuse、material factors、render state 和 `RenderQueue` 15 draw 展开。
- `ark_forward_pass_pipeline_smoke` 已新增 material grid uniform capture，覆盖 15 个 draw 的 per-material roughness / metallic uniform、specular IBL enable flag 和 max mip。
- 默认 specular IBL quality policy 已记录：prefiltered specular cube `256x256 RGBA16Float` full mip chain（9 mips）、prefilter sample count `128`、BRDF LUT `256x256 RGBA16Float`、BRDF LUT sample count `1024`。
- Phase 0.44 收尾时仍缺完整 screenshot/golden image system、tone-mapped LDR readback、public quality config API、shadow、多光源、bloom 和 auto exposure。

### Phase 0.45（0.45.0 ~ 0.45.6 已完成并验证）

- 新增 `docs/phase/phase45.md`，明确本阶段只做 frame color readback 与最小统计型视觉验证 foundation，不做完整 screenshot/golden image system、PNG writer、image diff 或 CI artifact。
- `VulkanCommandContext` 的 texture copy/upload bytes-per-pixel helper 已补齐 `RGBA16Float = 8 bytes`，让 HDR scene color 可以复用现有 `TextureReadbackDesc` 路径。
- `ark_readback_api_smoke` 已覆盖 `RGBA16Float` readback contract，约束 `bytesPerPixel = 8`。
- 新增 `ark_frame_validation_smoke`，创建真实 Vulkan backend、隐藏 GLFW window、`256x144 RGBA16Float` offscreen scene color、`D32Float` depth 和 `GpuToCpu` readback buffer。
- frame validation smoke 会加载 `specular_ibl_validation_fixture.gltf`，使用程序化 HDR environment 转 cubemap，固定 `RenderScene` / `RenderView` / lighting / camera，并执行 `SkyboxPass + ForwardPass`。
- readback 后在 CPU 端解码 half-float，统计 finite pixel、non-black ratio、RGB min/max/mean 和 luminance，用保守阈值拦截黑屏、全 clear、NaN/Inf 和异常亮度。
...
19. `docs/phase/phase39.md` / `docs/phase/phase40.md` / `docs/phase/phase41.md` / `docs/phase/phase42.md` / `docs/phase/phase43.md` / `docs/phase/phase44.md` / `docs/phase/phase45.md` / `docs/phase/phase46.md` / `docs/phase/phase47.md`
   - 确认 cubemap mip / face-mip view foundation、prefiltered specular environment generation foundation、BRDF LUT foundation、renderer default specular bake path、ForwardPass specular IBL 接入、specular IBL validation fixture、quality policy、frame color readback / statistical pixel validation、glTF camera / scene camera selection、material ball visual fixture、非目标和验证记录。
...
22. `tests/readback_api_smoke.cpp` / `tests/cubemap_orientation_pixel_smoke.cpp` / `tests/frame_validation_smoke.cpp`
   - 看 fake API contract、readback-enabled cubemap usage、真实 Vulkan cubemap center pixel validation 和 offscreen HDR scene color statistical pixel validation。
...
- cubemap RHI/Vulkan 语义、equirectangular -> cubemap conversion、diffuse irradiance generation、ForwardPass diffuse IBL、face orientation debug foundation、最小 GPU readback / pixel validation、face-mip render target view foundation、prefiltered specular generator foundation、BRDF LUT foundation、默认 renderer specular bake path、ForwardPass specular IBL、roughness/metallic validation fixture、默认 specular quality policy、最小 HDR scene color statistical frame validation 和 tone-mapped LDR readback 已存在，但当前没有 cubemap CPU upload、cubemap mip generation policy 或 screenshot/golden validation。
...
本轮完成 `ForwardPass` specular IBL 最小接入：mesh shader 已消费 default renderer 传入的 prefiltered specular cubemap 和 BRDF LUT，并在缺少真实资源时绑定 fallback descriptor 但通过 `environment.w = 0` 禁用 specular contribution。Phase 0.43 收尾时仍没有 screenshot/golden image validation、roughness/metallic 视觉 fixture 或 specular quality policy；其中 quad-grid fixture 和默认 quality policy 已在 Phase 0.44 补齐。

Phase 0.44 收尾在 Windows/MSVC/vcpkg/DXC debug preset 下完成验证：
...
本轮完成 specular IBL validation / quality pass：新增 roughness/metallic quad-grid fixture，覆盖 asset -> ModelResource -> RenderQueue -> ForwardPass material uniform 的数据链路，并记录默认 specular IBL quality policy。它仍没有完整 screenshot/golden image system、frame color readback validation 或球体材质视觉 fixture。

Phase 0.45 收尾在 Windows/MSVC/vcpkg/DXC debug preset 下完成验证：
...
Phase 0.47 收尾在 Windows/MSVC/vcpkg/DXC debug preset 下完成验证：
...
本轮完成 material ball visual fixture foundation：新增 `assets/models/material_ball_validation_fixture.gltf`，提供 3 档 metallic x 5 档 roughness 的 15 个材质球、稳定 `MaterialBallCamera`、共享 sphere accessors、loader/model/resource/render queue coverage，并把 `ark_frame_validation_smoke` 扩展为同时渲染旧 specular quad-grid fixture 和新 material ball fixture。它仍不是完整 screenshot/golden image system，也不读取 tone-mapped LDR target；ForwardPass fake-RHI smoke 继续覆盖 15 格 roughness/metallic uniform，真实 fixture 通过 ModelResource/RenderQueue 和 Vulkan frame validation 覆盖。

## 8. 推荐下一步

Phase 0.48 已完成 tone-mapped LDR readback foundation。下一阶段建议沿着“可回归的最终画面验证”继续推进，并开始整理 renderer/sandbox 的 public scene loading 边界：

- 若继续推进自动化画面检查：基于现有 LDR offscreen readback 增加 PNG/screenshot capture、baseline 管理和 image diff。
- 若继续推进工程边界：设计 public scene/resource loading API，把模型、环境、相机和 quality config 从 sandbox 特例逐步整理成 renderer 可复用入口。
- 若继续推进视觉质量：在有 screenshot/golden 前，不建议先扩 Bloom 或 auto exposure；先保证当前 PBR/IBL 输出能稳定被最终画面验证捕捉。

不要直接跳到 RenderGraph、bindless、完整材质扩展或复杂后处理栈。

优先顺序：

1. Screenshot / golden image system，基于稳定相机、quad-grid/material-ball fixtures 和 LDR readback harness 扩展 PNG/baseline/image diff。
2. Public scene/resource loading API，把默认模型、环境、相机和材质资源加载从 sandbox 特例整理成公开入口。
3. Public specular quality config API，等 renderer config / scene loading API 更清楚后再做。
4. HDR/cubemap mip generation policy。
5. Bloom / auto exposure / ACES tone mapping。
