...
- 当前仍不是完整 production IBL；尚缺 screenshot/golden image 验证、frame color readback validation、球体材质视觉 fixture、public quality config API、shadow、多光源、bloom 或 auto exposure。
...
本轮 0.44.0 ~ 0.44.6 收尾主要工作区改动：

```text
## main...origin/main
 M docs/codex_handoff.md
 M docs/phase/phase44.md
 M tests/forward_pass_pipeline_smoke.cpp
 M tests/gltf_loader_smoke.cpp
 M tests/model_resource_smoke.cpp
?? assets/models/specular_ibl_validation_fixture.gltf
```
...
- 当前仍缺完整 screenshot/golden image system、frame color readback validation、球体材质视觉 fixture、public quality config API、glTF camera、shadow、多光源、bloom 和 auto exposure。

## 4. 关键代码阅读顺序

建议按以下顺序审核当前 Phase 0.44 完整闭环：
...
19. `docs/phase/phase39.md` / `docs/phase/phase40.md` / `docs/phase/phase41.md` / `docs/phase/phase42.md` / `docs/phase/phase43.md` / `docs/phase/phase44.md`
   - 确认 cubemap mip / face-mip view foundation、prefiltered specular environment generation foundation、BRDF LUT foundation、renderer default specular bake path、ForwardPass specular IBL 接入、specular IBL validation fixture、quality policy、非目标和验证记录。
...
- cubemap RHI/Vulkan 语义、equirectangular -> cubemap conversion、diffuse irradiance generation、ForwardPass diffuse IBL、face orientation debug foundation、最小 GPU readback / pixel validation、face-mip render target view foundation、prefiltered specular generator foundation、BRDF LUT foundation、默认 renderer specular bake path、ForwardPass specular IBL、roughness/metallic validation fixture 和默认 specular quality policy 已存在，但当前没有 cubemap CPU upload、cubemap mip generation policy、frame color readback validation 或 screenshot/golden validation。
...
Phase 0.44 已完成 specular IBL validation fixture 和默认 quality policy。下一阶段建议优先补 frame-level validation 或稳定相机入口：

- 若继续推进验证基础：基于 Phase 0.38 readback 扩展 frame color screenshot / golden image / statistical pixel smoke，至少覆盖 skybox + validation fixture + DamagedHelmet。
...
1. Screenshot / frame color readback / golden image or statistical pixel infrastructure。
2. glTF camera / scene camera selection。
3. Sphere/material ball validation fixture，补足 quad-grid fixture 的视觉不足。
