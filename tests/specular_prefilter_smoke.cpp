...
namespace {
    struct alignas(16) CapturedSpecularPrefilterUniform {
        ark::u32 faceIndex = 0;
        ark::u32 mipLevel = 0;
    ...
    static_assert(sizeof(CapturedSpecularPrefilterUniform) == 16);
    ...
    ark::u32 faceMipIndex(ark::u32 faceIndex, ark::u32 mipLevel) {
        return mipLevel * ark::EnvironmentCubeResource::FaceCount + faceIndex;
    }
    ...
    public:
        ...
        bool updateBuffer(ark::rhi::Buffer& buffer, const void* data, ark::u64 size, ark::u64 offset = 0) override {
            if (buffer.getDesc().debugName == "SpecularPrefilterUniformBuffer") {
                if (size != sizeof(CapturedSpecularPrefilterUniform) || offset != 0) {
                    return false;
                }

                CapturedSpecularPrefilterUniform uniform{};
        ...
        std::vector<CapturedSpecularPrefilterUniform> uniforms;
    ...
    bool validateDescriptorLayout(const ark::rhi::DescriptorSetLayoutDesc& desc) {
        if (desc.debugName != "SpecularPrefilterDescriptorSetLayout" || desc.bindings.size() != 3) {
            std::cerr << "EnvironmentSpecularPrefilterGenerator descriptor layout shape is invalid\n";
            return false;
        }
        ...
        if (uniformBinding.binding != 0 ||
            ...
            std::cerr << "EnvironmentSpecularPrefilterGenerator descriptor bindings are invalid\n";
            return false;
        }
    ...
    bool validateResourceSetup(const FakeRenderDevice& device) {
        if (device.shaderDescs.size() != 2 || device.pipelineLayoutDescs.size() != 1) {
            std::cerr << "EnvironmentSpecularPrefilterGenerator shader or pipeline layout resources are invalid\n";
            return false;
        }
    ...
    bool validateExecution(const FakeRenderDevice& device,
                           ...
                           const ark::EnvironmentCubeResource& source,
                           const ark::EnvironmentCubeResource& target) {
        constexpr ark::u32 MipLevels = 4;
        constexpr ark::u32 FaceCount = ark::EnvironmentCubeResource::FaceCount;
        constexpr ark::u32 FaceMipCount = FaceCount * MipLevels;
        ...
        if (context.resourceBarriers.size() != 2 ||
            ...
            std::cerr << "EnvironmentSpecularPrefilterGenerator did not transition target cubemap correctly\n";
            return false;
        }

        if (context.beginRenderingCount != static_cast<int>(FaceMipCount) ||
            context.endRenderingCount != static_cast<int>(FaceMipCount) ||
            context.pipelineBinds != static_cast<int>(FaceMipCount) ||
            context.descriptorBinds != static_cast<int>(FaceMipCount) ||
            context.draws != static_cast<int>(FaceMipCount) ||
            context.drawDescs.size() != FaceMipCount ||
            context.renderingDescs.size() != FaceMipCount ||
            context.viewports.size() != FaceMipCount ||
            context.scissors.size() != FaceMipCount ||
            context.uniforms.size() != FaceMipCount) {
            std::cerr << "EnvironmentSpecularPrefilterGenerator did not render every face mip\n";
            return false;
        }

        for (ark::u32 mipLevel = 0; mipLevel < MipLevels; ++mipLevel) {
            const ark::rhi::Extent2D mipExtent = target.mipExtent(mipLevel);
            const float expectedRoughness = static_cast<float>(mipLevel) / static_cast<float>(MipLevels - 1);
            for (ark::u32 faceIndex = 0; faceIndex < FaceCount; ++faceIndex) {
                const ark::u32 index = faceMipIndex(faceIndex, mipLevel);
                if (context.drawDescs[index].vertexCount != 3 ||
                    context.renderingDescs[index].extent.width != mipExtent.width ||
                    context.renderingDescs[index].extent.height != mipExtent.height ||
                    context.renderingDescs[index].colorAttachment.view !=
                        target.faceMipRenderTargetView(faceIndex, mipLevel) ||
                    context.viewports[index].width != static_cast<float>(mipExtent.width) ||
                    context.viewports[index].height != static_cast<float>(mipExtent.height) ||
                    context.scissors[index].width != mipExtent.width ||
                    context.scissors[index].height != mipExtent.height ||
                    context.uniforms[index].faceIndex != faceIndex ||
                    context.uniforms[index].mipLevel != mipLevel ||
                    context.uniforms[index].sampleCount != 16 ||
                    !nearlyEqual(context.uniforms[index].roughness, expectedRoughness)) {
                    std::cerr << "EnvironmentSpecularPrefilterGenerator per-face-mip command data is invalid\n";
                    return false;
                }
            }
        }

        std::size_t uniformBuffers = 0;
        for (const ark::rhi::BufferDesc& bufferDesc : device.bufferDescs) {
            if (bufferDesc.debugName == "SpecularPrefilterUniformBuffer") {
                ++uniformBuffers;
                if (bufferDesc.size != sizeof(CapturedSpecularPrefilterUniform) ||
                    bufferDesc.usage != ark::rhi::BufferUsage::Uniform ||
                    bufferDesc.memoryUsage != ark::rhi::MemoryUsage::CpuToGpu) {
                    std::cerr << "EnvironmentSpecularPrefilterGenerator uniform buffer desc is invalid\n";
                    return false;
                }
            }
        }

        if (uniformBuffers != FaceMipCount || device.descriptorSets.size() != FaceMipCount) {
            std::cerr << "EnvironmentSpecularPrefilterGenerator did not create one resource set per face mip\n";
            return false;
        }

        if (device.pipelineDescs.size() != 1) {
            std::cerr << "EnvironmentSpecularPrefilterGenerator did not create exactly one pipeline\n";
            return false;
        }

        const ark::rhi::GraphicsPipelineDesc& pipelineDesc = device.pipelineDescs.front();
        if (pipelineDesc.debugName != "SpecularPrefilterPipeline" ||
            ...
            std::cerr << "EnvironmentSpecularPrefilterGenerator pipeline state is invalid\n";
            return false;
        }
        ...
        for (const FakeDescriptorSet* descriptorSet : device.descriptorSets) {
            if (descriptorSet->capture.uniformBufferUpdates != 1 ||
                ...
                descriptorSet->capture.lastUniformRange != sizeof(CapturedSpecularPrefilterUniform) ||
                ...
                descriptorSet->sourceImageView != source.textureView() ||
                descriptorSet->sourceSampler != source.sampler()) {
                std::cerr << "EnvironmentSpecularPrefilterGenerator descriptor set updates are invalid\n";
                return false;
            }
        }
    ...
    bool validateSpecularPrefilterGeneration() {
        FakeRenderDevice device{};
        FakeDeviceContext context{};

        ark::EnvironmentCubeResource source{};
        ark::EnvironmentCubeResourceDesc sourceDesc{};
        sourceDesc.debugName = "SpecularPrefilterSourceCube";
        sourceDesc.faceExtent = ark::rhi::Extent2D{64, 64};
        sourceDesc.format = ark::rhi::Format::RGBA16Float;
        sourceDesc.mipLevels = 1;
        if (!source.create(device, sourceDesc)) {
            std::cerr << "Failed to create specular prefilter source cubemap\n";
            return false;
        }
        static_cast<FakeTexture*>(source.texture())->setState(ark::rhi::ResourceState::ShaderResource);

        ark::EnvironmentCubeResource target{};
        ark::EnvironmentCubeResourceDesc targetDesc{};
        targetDesc.debugName = "SpecularPrefilterTargetCube";
        targetDesc.faceExtent = ark::rhi::Extent2D{64, 64};
        targetDesc.format = ark::rhi::Format::RGBA16Float;
        targetDesc.mipLevels = 4;
        if (!target.create(device, targetDesc)) {
            std::cerr << "Failed to create specular prefilter target cubemap\n";
            return false;
        }

        ark::EnvironmentSpecularPrefilterGenerator generator{};
        ...
        ark::EnvironmentSpecularPrefilterDesc nullDesc{};
        nullDesc.debugName = "InvalidNullSpecularPrefilter";
        if (generator.generate(context, nullDesc)) {
            std::cerr << "EnvironmentSpecularPrefilterGenerator should reject null source/target\n";
            return false;
        }

        ark::EnvironmentSpecularPrefilterDesc sameCubeDesc{};
        sameCubeDesc.source = &source;
        sameCubeDesc.target = &source;
        sameCubeDesc.debugName = "InvalidSameCubeSpecularPrefilter";
        if (generator.generate(context, sameCubeDesc)) {
            std::cerr << "EnvironmentSpecularPrefilterGenerator should reject source == target\n";
            return false;
        }

        ark::EnvironmentCubeResource invalidStateSource{};
        ark::EnvironmentCubeResourceDesc invalidStateSourceDesc{};
        invalidStateSourceDesc.debugName = "SpecularPrefilterInvalidStateSource";
        invalidStateSourceDesc.faceExtent = ark::rhi::Extent2D{64, 64};
        invalidStateSourceDesc.format = ark::rhi::Format::RGBA16Float;
        invalidStateSourceDesc.mipLevels = 1;
        if (!invalidStateSource.create(device, invalidStateSourceDesc)) {
            std::cerr << "Failed to create invalid-state specular prefilter source cubemap\n";
            return false;
        }
        ark::EnvironmentSpecularPrefilterDesc invalidStateDesc{};
        invalidStateDesc.source = &invalidStateSource;
        invalidStateDesc.target = &target;
        invalidStateDesc.debugName = "InvalidSourceStateSpecularPrefilter";
        if (generator.generate(context, invalidStateDesc)) {
            std::cerr << "EnvironmentSpecularPrefilterGenerator should reject non-ShaderResource source\n";
            return false;
        }
        invalidStateSource.resetImmediate();

        context = FakeDeviceContext{};
        ark::EnvironmentSpecularPrefilterDesc prefilterDesc{};
        prefilterDesc.source = &source;
        prefilterDesc.target = &target;
        prefilterDesc.sampleCount = 8;
        prefilterDesc.debugName = "SpecularPrefilterSmoke";
        if (!generator.generate(context, prefilterDesc)) {
            std::cerr << "EnvironmentSpecularPrefilterGenerator generation failed\n";
            return false;
        }

        if (!validateExecution(device, context, source, target)) {
            return false;
        }
...
int main() {
    return validateSpecularPrefilterGeneration() ? EXIT_SUCCESS : EXIT_FAILURE;
}
