...
namespace {
    ...
    bool validateMaterialBallValidationModelResource() {
        constexpr std::size_t RowCount = 3;
        constexpr std::size_t ColumnCount = 5;
        constexpr std::size_t MaterialCount = RowCount * ColumnCount;
        const std::array<float, RowCount> metallicValues{0.0f, 0.5f, 1.0f};
        ...
        const ark::Path modelPath = findModelPath(ark::Path{"assets/models/material_ball_validation_fixture.gltf"});
        if (modelPath.empty()) {
        ...
        if (modelData.empty() || modelData.meshes.size() != MaterialCount ||
            modelData.materials.size() != MaterialCount || modelData.instances.size() != MaterialCount) {
            std::cerr << "Unexpected material ball validation fixture model data\n";
            return false;
        }
        ...
        ark::ModelResource modelResource{};
        if (!modelResource.create(device, textureCache, modelData)) {
            std::cerr << "Material ball validation ModelResource create failed\n";
            return false;
        }
        ...
        if (modelResource.primitiveCount() != MaterialCount ||
            ...
            device.textureViewCount != 5 ||
            device.samplerCount != 5) {
            std::cerr << "Material ball validation ModelResource counts are invalid\n";
            return false;
        }
        ...
        for (std::size_t index = 0; index < MaterialCount; ++index) {
            ...
            ark::MeshResource* mesh = modelResource.primitiveMesh(index);
            if (!material || !mesh) {
                std::cerr << "Material ball validation primitive resources are missing\n";
                return false;
            }

            const ark::MaterialFactors& factors = material->factors();
            const std::array<float, 4>& baseColor = baseColorValues[row];
            if (!near(factors.baseColorFactor[0], baseColor[0]) ||
                !near(factors.baseColorFactor[1], baseColor[1]) ||
                !near(factors.baseColorFactor[2], baseColor[2]) ||
                !near(factors.baseColorFactor[3], baseColor[3]) ||
                !near(factors.metallicFactor, metallicValues[row]) ||
                !near(factors.roughnessFactor, roughnessValues[column])) {
                std::cerr << "Material ball validation material factors were not preserved\n";
                return false;
            }
            ...
            if (renderState.alphaMode != ark::asset::AlphaMode::Opaque ||
                !near(renderState.alphaCutoff, 0.5f) ||
                renderState.doubleSided) {
                std::cerr << "Material ball validation material render state is invalid\n";
                return false;
            }
        }

        ark::RenderScene scene{};
        scene.addModel(modelResource, glm::mat4{1.0f}, "MaterialBallValidationModel");

        ark::RenderQueue queue{};
        queue.build(scene);
        if (queue.size() != MaterialCount) {
            std::cerr << "Material ball validation queue size is invalid\n";
            return false;
        }
        ...
        for (std::size_t index = 0; index < MaterialCount; ++index) {
            if (drawItems[index].mesh != modelResource.primitiveMesh(index) ||
                drawItems[index].material != modelResource.primitiveMaterial(index)) {
                std::cerr << "Material ball validation queue draw item resources are invalid\n";
                return false;
            }
        }

        if (!near(drawItems.front().modelMatrix[3][0], -2.4f) ||
            !near(drawItems.front().modelMatrix[3][1], 1.1f) ||
            !near(drawItems.front().modelMatrix[0][0], 0.45f) ||
            !near(drawItems.back().modelMatrix[3][0], 2.4f) ||
            !near(drawItems.back().modelMatrix[3][1], -1.1f) ||
            !near(drawItems.back().modelMatrix[2][2], 0.45f)) {
            std::cerr << "Material ball validation queue transforms are invalid\n";
            return false;
        }
