...
namespace {
    struct alignas(16) CapturedIrradianceUniform {
        ark::u32 faceIndex = 0;
    ...
    static_assert(sizeof(CapturedIrradianceUniform) == 16);
    ...
    public:
        ...
        bool updateBuffer(ark::rhi::Buffer& buffer, const void* data, ark::u64 size, ark::u64 offset = 0) override {
            if (buffer.getDesc().debugName == "IrradianceUniformBuffer") {
                if (size != sizeof(CapturedIrradianceUniform) || offset != 0) {
                    return false;
                }

                CapturedIrradianceUniform uniform{};
        ...
        std::vector<CapturedIrradianceUniform> uniforms;
    ...
    bool validateDescriptorLayout(const ark::rhi::DescriptorSetLayoutDesc& desc) {
        if (desc.debugName != "IrradianceDescriptorSetLayout" || desc.bindings.size() != 3) {
            std::cerr << "EnvironmentIrradianceGenerator descriptor layout shape is invalid\n";
            return false;
        }
        ...
        if (uniformBinding.binding != 0 ||
            ...
            std::cerr << "EnvironmentIrradianceGenerator descriptor bindings are invalid\n";
            return false;
        }
    ...
    bool validateResourceSetup(const FakeRenderDevice& device) {
        ...
        for (const ark::rhi::BufferDesc& bufferDesc : device.bufferDescs) {
            if (bufferDesc.debugName == "IrradianceUniformBuffer") {
                ++uniformBuffers;
                if (bufferDesc.size != sizeof(CapturedIrradianceUniform) ||
                    bufferDesc.usage != ark::rhi::BufferUsage::Uniform ||
                    bufferDesc.memoryUsage != ark::rhi::MemoryUsage::CpuToGpu) {
                    std::cerr << "EnvironmentIrradianceGenerator uniform buffer desc is invalid\n";
                    return false;
                }
            }
        }

        if (uniformBuffers != FaceCount || device.descriptorSets.size() != FaceCount) {
            std::cerr << "EnvironmentIrradianceGenerator did not create one descriptor set and uniform per face\n";
            return false;
        }

        if (device.shaderDescs.size() != 2 || device.pipelineLayoutDescs.size() != 1) {
            std::cerr << "EnvironmentIrradianceGenerator shader or pipeline layout resources are invalid\n";
            return false;
        }
    ...
    bool validateExecution(const FakeRenderDevice& device,
        ...
        if (context.resourceBarriers.size() != 2 ||
            ...
            std::cerr << "EnvironmentIrradianceGenerator did not transition target cubemap correctly\n";
            return false;
        }
        ...
        if (context.beginRenderingCount != static_cast<int>(FaceCount) ||
            ...
            context.uniforms.size() != FaceCount) {
            std::cerr << "EnvironmentIrradianceGenerator did not render exactly six faces\n";
            return false;
        }
        ...
        for (std::size_t faceIndex = 0; faceIndex < FaceCount; ++faceIndex) {
            if (context.drawDescs[faceIndex].vertexCount != 3 ||
                ...
                context.uniforms[faceIndex].faceIndex != faceIndex ||
                context.uniforms[faceIndex].sampleDelta != 0.125f) {
                std::cerr << "EnvironmentIrradianceGenerator per-face command data is invalid\n";
                return false;
            }
        }

        if (device.pipelineDescs.size() != 1) {
            std::cerr << "EnvironmentIrradianceGenerator did not create exactly one pipeline\n";
            return false;
        }

        const ark::rhi::GraphicsPipelineDesc& pipelineDesc = device.pipelineDescs.front();
        if (pipelineDesc.debugName != "IrradiancePipeline" ||
            ...
            std::cerr << "EnvironmentIrradianceGenerator pipeline state is invalid\n";
            return false;
        }
        ...
        for (const FakeDescriptorSet* descriptorSet : device.descriptorSets) {
            if (descriptorSet->capture.uniformBufferUpdates != 1 ||
                ...
                descriptorSet->capture.lastUniformRange != sizeof(CapturedIrradianceUniform) ||
                ...
                std::cerr << "EnvironmentIrradianceGenerator descriptor set updates are invalid\n";
                return false;
            }
        }
    ...
    bool validateIrradianceGeneration() {
        ...
        if (!source.create(device, sourceDesc)) {
            std::cerr << "Failed to create irradiance source cubemap\n";
            return false;
        }

        ark::EnvironmentCubeResource target{};
        ark::EnvironmentCubeResourceDesc targetDesc{};
        targetDesc.debugName = "IrradianceTargetCube";
        ...
        if (!target.create(device, targetDesc)) {
            std::cerr << "Failed to create irradiance target cubemap\n";
            return false;
        }

        ark::EnvironmentIrradianceGenerator generator{};
        ...
        ark::EnvironmentIrradianceGenerationDesc invalidDesc{};
        invalidDesc.source = &source;
        invalidDesc.target = &source;
        invalidDesc.debugName = "InvalidSameCubeIrradiance";
        if (generator.generate(context, invalidDesc)) {
            std::cerr << "EnvironmentIrradianceGenerator should reject source == target\n";
            return false;
        }

        context = FakeDeviceContext{};
        ark::EnvironmentIrradianceGenerationDesc generationDesc{};
        generationDesc.source = &source;
        generationDesc.target = &target;
        generationDesc.sampleDelta = 0.125f;
        generationDesc.debugName = "IrradianceSmoke";
        if (!generator.generate(context, generationDesc)) {
            std::cerr << "EnvironmentIrradianceGenerator generation failed\n";
            return false;
        }
...
int main() {
    return validateIrradianceGeneration() ? EXIT_SUCCESS : EXIT_FAILURE;
}
